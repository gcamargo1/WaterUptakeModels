...
class Crop(object):
    ...
    def water_uptake_epic(self, soil):
        ...
        for lyr in soil.layers:
            BLM = (soil.perm_wilt_point[lyr] * soil.layer_thickness[lyr] *
                   WATER_DENSITY)
            FC = (soil.field_capacity[lyr] * soil.layer_thickness[lyr] *
                  WATER_DENSITY)
            ST = (soil.water_content[lyr] * soil.layer_thickness[lyr] *
                  WATER_DENSITY)
            XX = math.log(BLM)

            SUM[lyr] = (EP * (1 - math.exp(-UB1 * soil.cum_depth[lyr] / RD)) /
                        UOB)

            WTN = max(5,
                      10 ** (3.1761 - 1.6576 *
                      ((math.log(ST)-XX) / (math.log(FC) - XX))))
            XX = TOS + WTN
            if XX < 5000:
                F = 1 - XX / (XX + math.exp(SCRP211 - SCRP212 * XX))
                self.water_uptake[lyr] = (
                    min(SUM[lyr] - CPWU * self.water_uptake.sum() -
                        (1. - CPWU) * UX, ST - BLM) * RGS * F)
                if self.water_uptake[lyr] < 0:
                    self.water_uptake[lyr] = 0
            UX = SUM[lyr]
