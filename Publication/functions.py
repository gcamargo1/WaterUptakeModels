...
def vol_water_content_33_j_kg(clay, sand, organic_matter):
    ...
    0.38
    """
    x1 = (0.299 - 0.251 * sand + 0.195 * clay + 0.011 * organic_matter + 0.006
          * sand * organic_matter - 0.027 * clay * organic_matter + 0.452
          * sand * clay)
...
def vol_water_content_1500_jkg(clay, sand, organic_matter):
    ...
    0.21
    """
    x1 = (0.031 - 0.024 * sand + 0.487 * clay + 0.006 * organic_matter + 0.005 *
          sand * organic_matter - 0.013 * clay * organic_matter + 0.068 * sand *
          clay)
    return -0.02 + 1.14 * x1
...
def b_value(water_content_33_j_kg, water_content_1500_j_kg):
    ...
    3.89
    """
    return ((math.log(1500) - math.log(33)) / (math.log(water_content_33_j_kg)
             - math.log(water_content_1500_j_kg)))
...
def water_potential(sat_water_content, air_entry_potential, campbell_b,
                     water_content):
    ...
    return (air_entry_potential * (sat_water_content / water_content)
            ** campbell_b)

def water_content(sat_water_content, air_entry_potential, campbell_b,
                  water_potential):
    """ (float,float,float) -> (float)
...
def feddes_stress_factor(transp_pot, water_pot, water_pot_sat,
                         water_pot_field_cap, water_pot_stress_low_t,
                         water_pot_stress_high_t, water_pot_wilting,
    ...
    if water_pot > water_pot_wilting and water_pot < water_pot_stress:
        return ((water_pot - water_pot_wilting) /
                (water_pot_stress - water_pot_wilting))
    # Optimal conditions
    ...
    if water_pot > water_pot_field_cap and water_pot < water_pot_sat:
        return ((water_pot - water_pot_sat) /
                (water_pot_field_cap - water_pot_sat))
...
def p_wofost(ET, drought_cat):
    ...
    A = 0.76
    B = 1.5
    stress_fact = 1./(A + B * ET) - (5.-drought_cat) * 0.10
    if drought_cat < 3:
        stress_fact = (stress_fact + (ET - 0.6) /
                       (drought_cat * (drought_cat + 3.)))
    if stress_fact < 0.1:
        stress_fact = 0.1
    if stress_fact > 0.95:
        stress_fact = 0.95
...
def vapor_pressure_air(vapor_pressure_temp_min, vapor_pressure_temp_max, rh_max,
                       rh_min):
    """
    ...
    1.455
    """
    return 0.5 * (vapor_pressure_temp_min * rh_max / 100.
                  + vapor_pressure_temp_max * rh_min / 100.)
...
def vapor_press_defct_ave(max_sat_vap_press, min_sat_vap_press, air_vap_press):
    """max_sat_vap_press: max sat vapor pressure - kPa
       ...
       """
    return (max_sat_vap_press + min_sat_vap_press) / 2. - air_vap_press

def vapor_press_defct_max(max_sat_vap_press, min_relative_humidity):
    '''max vapor presure deficit'''
    return 0.67 * max_sat_vap_press * (1 - min_relative_humidity / 100.)
